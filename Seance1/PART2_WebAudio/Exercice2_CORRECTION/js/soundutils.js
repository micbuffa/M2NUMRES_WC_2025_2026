async function loadAndDecodeSound(url, ctx) {
   ...
   // load and decode all sounds, takes as parameter
    // an array of sound URLs and the audio context
    // returns the array of decoded sounds
    async function decodeAllSounds(arrayOfSoundURLs, ctx) {
        // Decode all sounds
        let promiseArray = arrayOfSoundURLs.map(url => loadAndDecodeSound(url, ctx));
        let decodedSounds = await Promise.all(promiseArray);
  ...
  // export the function
  export { loadAndDecodeSound, decodeAllSounds, playSound };